...
let quizTimer = null;
let currentPracticeTask = parseInt(localStorage.getItem('currentPracticeTask')) || 1;
...
function loadCurrentTask() {
    ...
    document.getElementById('task-description').innerHTML = `<p>${task.description}</p><p><strong>Hint:</strong> ${task.hint}</p>`;
...
function nextTask() {
    if (currentPracticeTask < practiceTasks.length) {
        ...
        updateProgress();
        clearEditor();
        document.getElementById('results-container').innerHTML = '<p class="no-results">Run a query to see results here.</p>';
...
function loadQuestion(questionIndex) {
    ...
    radioButtons.forEach(radio => {
        radio.addEventListener('change', function() {
            quizAnswers[questionIndex] = parseInt(this.value);
            saveQuizState();
